...
static void RTreeGetBranches(struct Node *n, struct Branch *b)
{
	...
	CoverSplitArea = RTreeRectVolume(&CoverSplit);
...
static void RTreePickSeeds(struct PartitionVars *p)
{
	...
	for (i=0; i<p->total; i++)
		area[i] = RTreeRectVolume(&BranchBuf[i].rect);
	...
	for (i=0; i<p->total-1; i++)
	{
		for (j=i+1; j<p->total; j++)
		{
			struct Rect one_rect = RTreeCombineRect(
						&BranchBuf[i].rect,
						&BranchBuf[j].rect);
			waste = RTreeRectVolume(&one_rect) -
					area[i] - area[j];
			if (waste > worst)
			{
				worst = waste;
		...
		&& p->count[1] < p->total - p->minfill)
	{
		...
		for (i=0; i<p->total; i++)
		{
			if (!p->taken[i])
			{
				...
				growth0 = RTreeRectVolume(
						&rect_0)-p->area[0];
				growth1 = RTreeRectVolume(
						&rect_1)-p->area[1];
