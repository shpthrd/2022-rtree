...
static void RTreePrintFirstBranch(struct Branch *b, int depth)
{
	RTreePrintRect(&(b->rect), depth);
	RTreePrintFirst(b->child, depth);
}
...
void RTreePrintNode(struct Node *n, int depth)
{
	...
	for (i=0; i<n->count; i++)
	{
		if(n->level == 0) {
			RTreeTabIn(depth+1);
			printf("\t%d: data = %d\n", i, n->branch[i].child);
			RTreePrintRect(&n->branch[i], depth+1);
		}
		else {
			RTreeTabIn(depth+1);
			printf("branch %d\n", i);
...
//print just the first child of a node until leaf
void RTreePrintFirst(struct Node *n, int depth){
	...
	if(n->level > 0) {
		RTreeTabIn(depth);
		printf("branch %d\n", 0);
		RTreePrintFirstBranch(&n->branch[0], depth+1);
	}
