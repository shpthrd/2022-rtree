...
//caso um dos threads fique ocioso o proximo thread que encontrar um node a ser pesquisado o adiciona na fila
//a função auxiliar só finaliza quando o node Kill é encontrado, indicando que não há mais nada a ser pesquisado

void* RTreeSearchParalela(void* arg){
  Data* data = (Data*) arg;
  ...
  *(data->time_wait) = 0;
  ...
  while(1){
    ...
    *(data->time_wait) += (double)diff/NANOS;
    ...
    if(n->level != -1){
      clock_gettime(CLOCK_REALTIME, &tick);
      RTreeSearchParalela(data);
      clock_gettime(CLOCK_REALTIME, &tock);
      diff = NANOS * (tock.tv_sec - tick.tv_sec) + tock.tv_nsec - tick.tv_nsec;
      *(data->time_search) += (double)diff/NANOS;
    }
    else{
      break;
    }
...
struct Node * ArtificialRTree(int start,int end,int level){
  ...
  if(level >= 1){
    //printf("if level >= 1\n");
    int pace = size/MAXCARD;
    n->count = MAXCARD;
    for(i=0;i<MAXCARD-1;i++){
      for(j=0;j<NUMSIDES/2;j++){
        n->branch[i].rect.boundary[j] = start + (i * pace);
      }
      for(j=NUMSIDES/2;j<NUMSIDES;j++){
        n->branch[i].rect.boundary[j] = start + ((1+i) * pace);
      }
      n->branch[i].child = ArtificialRTree(start+(i * pace),start + ((1 + i) * pace),level - 1);
    }
    for(j=0;j<NUMSIDES/2;j++){
        n->branch[i].rect.boundary[j] = start + (i * pace);
      }
      for(j=NUMSIDES/2;j<NUMSIDES;j++){
        n->branch[i].rect.boundary[j] = end;
      }
      n->branch[i].child = ArtificialRTree(start+(i * pace),end,level - 1);

    //printf("end if level >= 1\n");
  }
  else{//level = 0 leaf
    //printf("else level = 0\n");
    //printf("start: %d\tend: %d\n",start,end);
    ...
    for(i=0;i<size;i++){
      for(j=0;j<NUMSIDES/2;j++){
        n->branch[i].rect.boundary[j] = start + i;
      }
      for(j=NUMSIDES/2;j<NUMSIDES;j++){
        n->branch[i].rect.boundary[j] = start + 1+i;
      }
      n->branch[i].child = (struct Node *) start + i;
      //printf("n->branch[%d].child: %d\n",i,n->branch[i].child);
      //printf("start + i: %d\n",start+i);
      //printf("rect: ");
      //int k;
      //for(k=0;k<NUMSIDES;k++) printf("%d ",n->branch[i].rect.boundary[k]);
      //printf("\n");
    }
    
  }
  //printf("before return n level %d\n",level);
  //printf("after all n-> level: %d\n",n->level);
  return n;
}
